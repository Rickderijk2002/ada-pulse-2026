...
def format_to_gold(res, freq_code, freq_label, domain, unit_map):
    ...
    # Melt to Long Format
    long_df = res.melt(id_vars=['period_start', 'period_end'], var_name='metric_name', value_name='metric_value')

    # Replace inf and NaN with zero
    long_df['metric_value'] = long_df['metric_value'].replace([float('inf'), float('-inf')], 0).fillna(0)
...
def compute_kpis(cloud_event: CloudEvent):
    ...
    try:
        financial_data = load_csv_from_gcs(bucket_name, object_path.replace("ready.json", FINANCIAL_CSV))
        sales_marketing_data = load_csv_from_gcs(bucket_name, object_path.replace("ready.json", SALES_MARKETING_CSV))
    ...
    try:
        ...
        weekly_sales_marketing = get_sales_marketing_kpis(sales_marketing_data, 'W', 'weekly')
        monthly_sales_marketing = get_sales_marketing_kpis(sales_marketing_data, 'ME', 'monthly')
        gold_kpi_snapshots = pd.concat([weekly_financial, monthly_financial, weekly_sales_marketing, monthly_sales_marketing], ignore_index=True)
